...
class materials {
    public:
        virtual bool scatter(
            const ray& r_in, const recordhit& rec, color& attenuation, ray& scattered
        ) const = 0;
};
...
class dielectric : public materials {
    public:
        ...
        virtual bool scatter(
            const ray& r, const recordhit& rec, color& attenuation, ray& scattered
        ) const override {
            attenuation = color(1.0, 1.0, 1.0);
            double refraction_ratio = rec.front ? (1.0/ir) : ir;

            vec3 unitdir = unit_vector(r.direction());
            double cos_theta = fmin(dot(-unitdir, rec.normal), 1.0);
            ...
            if (cannot_refract || reflectance(cos_theta, refraction_ratio) > random_double())
                ...
                direction = refract(unitdir, rec.normal, refraction_ratio);
